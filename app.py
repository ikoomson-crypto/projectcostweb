...
def load_currency_rates():
    """Load currency exchange rates from file or return defaults"""
    ...
    if os.path.exists(rates_file):
        try:
            ...
            return get_default_exchange_rates()
    else:
...
def get_default_exchange_rates():
    ...
    return {
        'USD': 1.0, 'EUR': 0.85, 'GBP': 0.73, 'JPY': 110.0, 'CNY': 6.45, 'INR': 74.0,
        'AUD': 1.35, 'CAD': 1.25, 'CHF': 0.92, 'SGD': 1.35, 'HKD': 7.78, 'NZD': 1.43,
        'KRW': 1150.0, 'RUB': 73.5, 'BRL': 5.25, 'ZAR': 14.5, 'AED': 3.67, 'SAR': 3.75,
        'MYR': 4.15, 'THB': 33.0, 'VND': 23000, 'IDR': 14400, 'PHP': 50.0, 'PKR': 160.0,
        'BDT': 85.0, 'NGN': 410.0, 'EGP': 15.7, 'TRY': 8.5, 'MXN': 20.0, 'SEK': 8.5,
        'NOK': 8.6, 'DKK': 6.3, 'PLN': 3.8, 'CZK': 21.5, 'HUF': 300.0, 'ILS': 3.2,
...
def format_currency(value, currency_code='USD', currency_symbol=None):
    """Format value with appropriate currency symbol and decimal places"""
    ...
    # Handle special currency symbol placements
    if currency_code in ['EUR', 'GBP', 'AUD', 'CAD', 'NZD', 'SGD', 'HKD', 'CHF', 'SEK', 'NOK', 'DKK']:
        return f"{currency_symbol}{formatted_value}"
    elif currency_code == 'INR':
        return f"{currency_symbol}{formatted_value}"
    elif currency_code == 'JPY':
        return f"{currency_symbol}{formatted_value}"
    elif currency_code in ['VND', 'IDR', 'KRW']:
...
def convert_project_items(project_data, new_currency):
    """Convert all costs in a project to a new currency"""
    ...
    if os.path.exists(PROJECTS_FILE):
        try:
            with open(PROJECTS_FILE, 'r') as f:
                ...
                if not content:
                    save_all_projects([])
                    print("Repaired empty projects.json")
                else:
                    json.loads(content)
        except json.JSONDecodeError:
            backup_file = f"{PROJECTS_FILE}.bak"
            if os.path.exists(backup_file):
                os.remove(backup_file)
            os.rename(PROJECTS_FILE, backup_file)
            save_all_projects([])
            print(f"Repaired corrupted projects.json, backup saved as {backup_file}")
    else:
        save_all_projects([])
...
def load_user_projects_assignment() -> Dict:
...
def save_user_projects_assignment(assignments: Dict):
    with open(USER_PROJECTS_FILE, 'w') as f:
        json.dump(assignments, f, indent=2)


def get_users_for_project(project_id: str) -> List[str]:
    assignments = load_user_projects_assignment()
    return assignments.get(project_id, [])
...
def save_all_projects(projects: List[Dict]):
...
def save_project_to_db(project_data: Dict, user_id: str) -> Dict:
    projects = load_all_projects()
    now = datetime.now().isoformat()
    if 'id' not in project_data or not project_data['id']:
        project_data['id'] = datetime.now().strftime('%Y%m%d%H%M%S%f')[:-3]
    ...
    project_data['modified_date'] = now
    project_data['user_id'] = user_id
    if 'expected_annual_benefit' not in project_data:
        project_data['expected_annual_benefit'] = 0
    if 'project_lifetime_years' not in project_data:
        project_data['project_lifetime_years'] = 3
    if 'discount_rate' not in project_data:
        project_data['discount_rate'] = 10
    if 'currency_code' not in project_data:
        project_data['currency_code'] = 'USD'
    if 'currency_symbol' not in project_data:
        project_data['currency_symbol'] = '$'
    if 'exchange_rate' not in project_data:
        project_data['exchange_rate'] = 1.0

    # Clean up None values
    for item in project_data.get('implementation_items', []):
        for key in ['rate', 'no_of_resource', 'allocation', 'days', 'months']:
        ...
        if item.get('location') is None:
            item['location'] = ''
        if item.get('workplace_type') is None:
            item['workplace_type'] = 'Remote'

    for item in project_data.get('software_items', []):
        for key in ['qty', 'unit_price', 'years']:
            if item.get(key) is None:
                item[key] = 0
    ...
    for item in project_data.get('logistics_items', []):
        for key in ['no_of_resources', 'round_trips', 'days_onsite', 'per_diem', 'accommodation', 'transport_per_day',
                    'flight_cost']:
            if item.get(key) is None:
                item[key] = 0
        for key in ['role', 'consultant_name', 'location_base']:
            if item.get(key) is None:
                item[key] = ''

    projects = [p for p in projects if p.get('id') != project_data.get('id')]
    projects.append(project_data)
    save_all_projects(projects)
    return project_data


def delete_project_from_db(project_id: str, user_id: str) -> bool:
    projects = load_all_projects()
    project = get_project_by_id(project_id)
    if project and (project.get('user_id') == user_id or get_user_by_id(user_id).get('role') == 'admin'):
...
def generate_pdf_report(project_data: Dict, report_type: str = 'full') -> io.BytesIO:
    ...
    if impl_items:
        ...
        story.append(Spacer(1, 0.2 * inch))
        story.append(Paragraph(
            f"<i>Implementation subtotal with {totals_with_markup['implementation_percent']:.1f}% markup: {format_currency(totals_with_markup['implementation_markup'], currency_code, currency_symbol)}</i>",
            normal_style))
        story.append(Spacer(1, 0.2 * inch))
    ...
    if sw_items:
        ...
        story.append(Spacer(1, 0.2 * inch))
        story.append(Paragraph(
            f"<i>Software subtotal with {totals_with_markup['software_percent']:.1f}% markup: {format_currency(totals_with_markup['software_markup'], currency_code, currency_symbol)}</i>",
            normal_style))
        story.append(Spacer(1, 0.2 * inch))
    ...
    if log_items:
        ...
        story.append(Spacer(1, 0.2 * inch))
        story.append(Paragraph(
            f"<i>Logistics subtotal with {totals_with_markup['logistics_percent']:.1f}% markup: {format_currency(totals_with_markup['logistics_markup'], currency_code, currency_symbol)}</i>",
            normal_style))
        story.append(Spacer(1, 0.2 * inch))
...
def import_from_excel(file, tab_type: str) -> List[Dict]:
    ...
    # Find the correct column names (they might have currency symbols)
    rate_col = None
    unit_price_col = None
    per_diem_col = None
...
def convert_project_currency(project_id):
    """Convert a project to a different currency"""
    ...
    # Convert all costs to new currency
...
def dashboard():
    ...
    settings = load_settings()
    return render_template('dashboard.html', user=session.get('username'), role=role, settings=settings)
...
def admin_panel():
    ...
    role = user.get('role', 'user') if user else 'user'
    settings = load_settings()
    return render_template('admin.html', user=session.get('username'), role=role, settings=settings)
...
def admin_settings():
    ...
    currencies = [{'code': code, 'name': info['name'], 'symbol': info['symbol']}
                  for code, info in SUPPORTED_CURRENCIES.items()]
    return render_template('admin_settings.html', user=session.get('username'), role='admin',
...
def admin_currencies():
    """Admin page for managing currency exchange rates"""
    user = get_user_by_id(session['user_id'])
    role = user.get('role', 'user') if user else 'user'
    settings = load_settings()
    ...
    # Prepare currencies list with rates
    currencies = []
    for code, info in SUPPORTED_CURRENCIES.items():
        currencies.append({
            'code': code,
...
def index():
    ...
    currencies = [{'code': code, 'symbol': info['symbol'], 'name': info['name']}
                  for code, info in SUPPORTED_CURRENCIES.items()]
    return render_template('index.html', user=session.get('username'), role=role,
                           settings=settings, currencies=currencies)
...
def projects_page():
    ...
    currencies = [{'code': code, 'symbol': info['symbol'], 'name': info['name']}
                  for code, info in SUPPORTED_CURRENCIES.items()]
    return render_template('projects.html', user=session.get('username'), role=role,
                           settings=settings, currencies=currencies)
...
if __name__ == '__main__':
    ...
    if not os.path.exists(USERS_FILE) or os.path.getsize(USERS_FILE) == 0:
        create_user('admin', 'admin123', 'admin@example.com', 'Administrator', 'admin')
        print("=" * 50)
    ...
    for user_data in [('viewer', 'viewer123', 'Viewer User', 'viewer'),
                      ('technical_lead', 'tech123', 'Technical Lead', 'technical_lead'),
                      ('procurement', 'proc123', 'Procurement Manager', 'procurement'),
                      ('ceo', 'ceo123', 'CEO', 'ceo')]:
        if not get_user_by_username(user_data[0]):
            create_user(user_data[0], user_data[1], f'{user_data[0]}@example.com', user_data[2], user_data[3])
